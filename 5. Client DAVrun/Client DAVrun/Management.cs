...
namespace Client_DAVrun
{
    public partial class Management : Form
    {
        ...
        {
            DialogResult result = MessageBox.Show("Завершить работу с Client DAVrun?", "Подтверждение", MessageBoxButtons.YesNo);
        ...
        private async void btnUploadFile_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string remoteFileName = Path.GetFileName(selectedFilePath) + "_" + selectedFileHash;    // Добавляем к имени файла символ "_" и хеш-сумму
        ...
        private void btnServInstall_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                foreach (var checkBox in checkedFiles)  // Цикл поочерёдной загрузки файлов
                {
                    ...
                    string targetFileName = checkBox.Text.Split('=').Last();    // Имя целевого файла после знака "="
                    string targetFilePath = targetPath + targetFileName;        // Путь к целевому файлу
                ...
                MessageBox.Show("Файлы успешно установлены.", "Информация", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                MessageBox.Show($"Ошибка при установке файлов: {ex.Message}", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void btnInstallRefresh_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (!files.Any())   // Если не нашли файлов по пути, выводим сообщение
                {
                    MessageBox.Show("В данный момент установок на ПК не производится.", "Информация", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            catch (Exception ex)
            {
        ...
        private void btnStopInstall_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Нет выбранных имён ПК для удаления.", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            //Выводим окно с подтверждением
            DialogResult result = MessageBox.Show("Отменить установку выбранных имён ПК?", "Отмена установки", MessageBoxButtons.YesNo);
            if (result != DialogResult.Yes)
            {
                return;
            }
            ...
            try
            {
                ...
                MessageBox.Show("Выбранные имена ПК успешно отменены.", "Информация", MessageBoxButtons.OK, MessageBoxIcon.Information);
