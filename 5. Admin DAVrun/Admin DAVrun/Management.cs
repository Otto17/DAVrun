...
using System.Threading;             // Библиотека предоставляет классы и методы для работы с потоками и синхронизацией
...
namespace Admin_DAVrun
{
    public partial class Management : Form
    {
        ...
        internal Management(SshClient sshClient, SftpClient sftpClient)
        {
            ...
            //Инициализация контрола "PictureBox" для анимации ожидания
            loadingSpinner = new PictureBox
            {
                Image = Properties.Resources.loading,   // Загружаем анимированную gif анимацию из ресурсов
                SizeMode = PictureBoxSizeMode.AutoSize, // Устанавливаем размер (автоматический размер под изображение)
                BackColor = Color.Transparent,          // Устанавливаем прозрачный фон
                Visible = false                         // По умолчанию анимация скрыта
            };

            this.Controls.Add(loadingSpinner);  // Добавляем созданный "PictureBox" в коллекцию элементов управления формы
            loadingSpinner.BringToFront();      // Отображаем поверх других элементов управления на форме
        }
        ...
        private async void btnSelectFile_Click(object sender, EventArgs e)
        {
            using (OpenFileDialog openFileDialog = new())                   // Создаём объект класса "OpenFileDialog"
            {
                if (openFileDialog.ShowDialog() == DialogResult.OK)         // Проверяем, была ли нажата кнопка "Открыть"
                {
                    ...
                    loadingSpinner.Location = new Point((this.ClientSize.Width - loadingSpinner.Width) / 2, (this.ClientSize.Height - loadingSpinner.Height) / 2);  // Устанавливаем gif анимацию по центру формы
                    loadingSpinner.Visible = true;  // Включаем видимость gif анимации

                    var cts = new CancellationTokenSource();    // Создаём новый экземпляр класса для управления задачами

                    var hashTask = Task.Run(() => ComputeFileHash(selectedFilePath), cts.Token);    // Вычисляем хеш-сумму файла из найденного пути

                    //Если задача не завершится за 500 мс, продолжим показывать анимацию
                    if (await Task.WhenAny(hashTask, Task.Delay(500)) != hashTask)
                    {
                        //Ожидаем завершения задачи
                        selectedFileHash = await hashTask;
                    }
                    else
                    {
                        //Задача завершилась быстро, получаем результат
                        selectedFileHash = hashTask.Result;
                    }

                    //Скрываем анимацию ожидания
                    loadingSpinner.Visible = false;

                    labelhash.Text = selectedFileHash;  // Показываем в другом label вычесленную хеш-сумму файла
                }
            }
        }
