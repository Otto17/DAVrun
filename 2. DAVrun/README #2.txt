...
- Возможность перед компиляцией выбрать повышенную защиту конфига (привязка к компьютеру), сделает его непереносимым на другие ПК, либо без привязки, но с меньшей защитой;
- Создание/изменение/удаление параметров зашифрованного конфиг файла осуществляется через аргументы командной строки;
- Потребление ОЗУ DAVrun при работе с большим файлом (во время загрузки и выполнения архива) не превышает 14Мб (написан на C);
...
3) Если требуется, то скачиваем библиотеки "libxml2" с сайта "http://xmlsoft.org/sources/win32/64bit/", качаем следующие архивы "iconv-1.14-win32-x86_64.7z", "libxml2-2.9.3-win32-x86_64.7z" и "zlib-1.2.8-win32-x86_64.7z".
Из архива "iconv-1.14-win32-x86_64.7z" с папки "bin" нужна библиотека "libiconv-2.dll", остальное копируем аналогично.
